...
The problem is the prerequisite gap. The readiness data tells the story starkly: 85% of organisations are already using generative AI, but only 22% say their architecture can support AI without modification (Insight-001, Economist Impact/Databricks survey, n=1,100). Most organisations cannot document their legacy architecture properly today — let alone maintain a structured, machine-readable EA repository that AI agents could operate on. The prerequisites for AI-augmented EA -- codified architecture standards, integration between EA tools and deployment pipelines, machine-readable repositories with consistent classification and tagging (source-026) -- are table stakes that most EA teams have not reached. The gap is not between current EA and AI-augmented EA. The gap is between current EA and baseline EA maturity.
