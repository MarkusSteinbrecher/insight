...
This synthesis draws from 56 sources published between 2024 and 2026, spanning practitioner articles, academic research, industry reports, analyst frameworks, a substantial body of consultancy research from BCG, Bain, Capgemini, EY, KPMG, and PwC, and two vendor-practitioner sources (Akka and Ardoq). The corpus was processed through a structured extraction pipeline that identified 164 canonical claims, 112 unique claims, and 24 direct contradictions. Each canonical claim was critically analyzed for novelty, actionability, and generality risk -- and the results remain sobering. Only 21 of the original 136 claims (15%) survived as genuine insights. The remaining 85% split between partial insights that need significant refinement, observations that are important but obvious to any experienced practitioner, and generalitys dressed in AI terminology.
...
The corpus comprises 56 sources: 16 practitioner-oriented articles from the original research, 7 academic papers, 5 industry reports, 3 blog posts (including source-055, a vendor architecture guide from Akka), 1 vendor whitepaper (source-056, Ardoq's prompt engineering playbook for enterprise architects), and 24 consultancy publications added in the expansion. The temporal concentration remains sharp -- the vast majority of sources were published between 2024 and 2026, reflecting how recently enterprise architecture has been forced to confront AI as a systemic concern rather than a point solution.
...
The reasoning approach dimension adds another layer to the right-sizing argument. Source-055 identifies three principal reasoning approaches — symbolic (rule-based, fast, cheap), chain-of-thought (LLM-based, flexible, expensive), and planning (goal-directed, structured) — each with distinct cost and flexibility tradeoffs (cc-163). Chain-of-thought reasoning can involve multiple LLM round-trips per task, while symbolic reasoning requires fewer tokens. Combined with source-055's claim that agentic AI tasks can be 100,000x more expensive than database costs (uc-124), the choice of reasoning approach becomes one of the highest-leverage cost decisions in agent architecture — analogous to model right-sizing but at the reasoning layer.
...
Source-055 provides a striking data point that makes the cost dimension tangible: agentic AI tasks can be 100,000x more expensive than database costs (uc-124). Even if the specific multiplier is approximate, it underscores that production AI economics are fundamentally different from traditional compute economics — and infrastructure planning that does not account for this order-of-magnitude shift will undershoot dramatically.
...
11. **Agent memory architecture and governance.** Source-055 introduces the short-term vs. long-term memory distinction, and source-001 discusses vectorized memory implementations, but no source addresses shared memory architectures for multi-agent systems, memory governance frameworks (staleness management, GDPR compliance for persistent agent memory), or the cost economics of context injection at scale. As agents move from single-task to persistent, multi-session operation, memory becomes an infrastructure concern on par with compute and storage.
