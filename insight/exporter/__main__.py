...
def main():
    ...
    parser.add_argument("--output", default=str(ROOT / "site" / "static" / "data"), help="Output directory")
    ...
    if args.topic:
        ...
        export_audit(args.topic, graph, args.output, kb_path=kb_path)
        print(f"Exported topic: {args.topic}")
    else:
        result = export_all(graph, args.output, kb_path=kb_path)
        # Export audit data for all topics
        from insight.exporter.export import _get_all_topics
        for slug in _get_all_topics(graph):
            export_audit(slug, graph, args.output, kb_path=kb_path)
